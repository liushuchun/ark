...
func PutFileWithProgress(l rpc.Logger, ret interface{}, uptoken, key string, data io.Reader, fsize int64, extra *PutExtra, events qio.UploadEvents) error {

	onProgress := func(filesize, uploadedBytes int64) {
		events.OnProgress(filesize, uploadedBytes)
	}

	f, err := qio.OpenUpFile(data, fsize, onProgress)
	if err != nil {
		return err
	}
