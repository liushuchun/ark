...
func testResume(size int32, bufSize int32, t *testing.T) {
	...
	for read != size {
		var pos int32 = 0
		if size-read < bufSize {
			pos = bufSize - (size - read)
		}
		cur := buf[pos:]
		n, _ := rand.Read(cur)
		...
		if !bytes.Equal(s.Sum(nil), r.Sum(nil)) {
			t.Fatalf("testResume failed with size:", size)
		}
...
func TestResume(t *testing.T) {
	testResume(53, 26, t)
	testResume(999, 1000, t)
	testResume(1024, 1050, t)
	testResume(1050, 36, t)
	testResume(1024*1024, 5*1024+333, t)
	testResume(6*1024*1024, 4*1024, t)
	testResume(13*1024*1024, 1024*1024+1023, t)

	{
		...
		buf := make([]byte, 25)
		copy(buf[:], "1234567890098765432112345")
		s.Write(buf[0:10])
		r.Write(buf[0:10])

		var data [96]byte
		r.ToBytes(&data)
		fmt.Println("tobytes:", &data)
		r = NewFromBytes(&data)
		s.Write(buf[10:25])
		r.Write(buf[10:25])
		fmt.Println("25 bytes s sum:", s.Sum(nil))
		fmt.Println("25 bytes r sum:", r.Sum(nil))
	}
}
