...
// Uint32Set is a set of strings, implemented via map[uint32]struct{} for minimal memory consumption.
type Uint32Set map[uint32]empty

// NewUint32Set creates a Uint32Set from a list of values.
...
func (s1 Uint32Set) IsSuperset(s2 Uint32Set) bool {
	for item := range s2 {
		if !s1.Has(item) {
			return false
		}
