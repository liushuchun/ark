...
func TestMgoProxy(t *testing.T) {

	services, err := NewStaticServices(
		`
	{
		"rs.master.mongo": {
	...
	`)
	if err != nil {
		t.Fatal("NewStaticServices failed:", err)
	}
	...
	p := New(services)
	err = p.AddProxy(&ProxyConf{
		Service: "rs.master.mongo",
		As:      "tcp://" + ln.Addr().String(),
	})
	if err != nil {
		t.Fatal("AddProxy failed:", err)
	}

	session, err := mgo.Dial(ln.Addr().String())
	if err != nil {
		t.Fatal("mgo.Dial failed:", err)
	}
	defer session.Close()

	c := session.DB("TestMgoProxy_mgotable").C("test")
	c.RemoveAll(M{})

	cfg := mgotable.NewConfig(new(Row)).
		WithUniques(new(ById), new(ByPidName)).
		WithIndexes(new(ByPid))

	coll, err := mgotable.New(c, cfg)
	if err != nil {
		t.Fatal("mgotable.New failed:", err)
	}
