...
func TestBigArray(t *testing.T) {
	...
	p, err := OpenBigArray(tbl, 2)
	if err != nil {
		ts.Fatal(t, "openBigArray failed:", err)
	}
	defer p.Close()
...
func TestBigArrayRandomWrite(t *testing.T) {

	modess := [][]int{{writeOk}}

	elemLen := 8
	tbl, err := openTable(modess, elemLen, 0)
	if tbl == nil || err != nil {
		ts.Fatal(t, "openTable failed:", err)
	}
	p, err := OpenBigArray(tbl, 2)
	...
	pstr := "12345678"
	if err := p.Put(2, []byte(pstr)); err != nil {
		ts.Fatal(t, "Put failed:", errors.Detail(err))
	}

	rbuf := make([]byte, elemLen)
	if err := p.Get(2, rbuf); err != nil {
		ts.Fatal(t, "Get failed:", errors.Detail(err))
	}
	if string(rbuf) != pstr {
		ts.Fatal(t, "Get unexpected -", string(rbuf))
	}
}
