...
func (t *Transport) RoundTrip(req *http.Request) (resp *http.Response, err error) {
	...
	if body != nil {
		if _, ok := body.(io.ReaderAt); !ok {
			req.Body = &onceReader{ReadCloser: body}
		}
	}
	...
	for i := uint32(0); i < tryTimes; i++ {
		...
		if t.ShouldReproxy(code, err) {
			...
			if ra, ok := body.(io.ReaderAt); ok {
				req.Body = readCloser{&cc.Reader{ra, 0}, body}
			} else if or, ok := req.Body.(*onceReader); ok {
				if or.read {
					return
				}
			} else if req.Body != nil { //must nil
				return
			}
			proxy = sel.Get(xl)
			if proxy == nil {
...
func (r *onceReader) Read(p []byte) (n int, err error) {
	n, err = r.ReadCloser.Read(p)
	if n > 0 {
		r.once.Do(func() { r.read = true })
	}
	return
}
