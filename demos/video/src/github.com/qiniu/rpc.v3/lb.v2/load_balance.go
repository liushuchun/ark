...
func rewindReader(r io.ReaderAt) io.Reader {
	return &qio.Reader{r, 0}
}

func (p *Client) Do(
	ctx context.Context, req *Request) (resp *http.Response, err error) {
	...
	if req.Body != nil {
		rc = ioutil.NopCloser(rewindReader(req.Body))
	}
...
func (p *Client) CallWith64(
	ctx context.Context, ret interface{}, method, path string, bodyType string, body io.ReaderAt, bodyLength int64) (err error) {
	...
	for i := 0; i < int(p.tryTimes); i++ {
		...
		fromIdx = pickIdx
		err = p.client.CallWith64(ctx, ret, method, c.host+path, bodyType, rewindReader(body), bodyLength)
		if err != nil && p.shouldRetry(err) {
			atomic.StoreInt64(&c.lastFailedTime, time.Now().Unix())
			continue
		}
...
func (p *Client) CallWith(ctx context.Context, ret interface{},
	method, path string, bodyType string, body io.ReaderAt, bodyLength int) (err error) {
	...
	for i := 0; i < int(p.tryTimes); i++ {
		...
		fromIdx = pickIdx
		err = p.client.CallWith(ctx, ret, method, c.host+path, bodyType, rewindReader(body), bodyLength)
		if err != nil && p.shouldRetry(err) {
			atomic.StoreInt64(&c.lastFailedTime, time.Now().Unix())
			continue
		}
...
func (p *Client) DoRequestWith(ctx context.Context, method, path, bodyType string,
	body io.ReaderAt, bodyLength int) (resp *http.Response, err error) {
	...
	for i := 0; i < int(p.tryTimes); i++ {
		...
		fromIdx = pickIdx
		resp, err = p.client.DoRequestWith(ctx, method, c.host+path, bodyType, rewindReader(body), bodyLength)
		if err != nil && p.shouldRetry(err) {
