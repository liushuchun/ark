...
const (
	...
	Fdate         = 1 << iota                     // the date: 2009/0123
	Ftime                                         // the time: 01:23:23
	...
	Fmodule                                       // module name
	Flevel                                        // level: 0(Debug), 1(Info), 2(Warn), 3(Error), 4(Panic), 5(Fatal)
	FstdFlags     = Fdate | Ftime | Fmicroseconds // initial values for the standard logger
	Fdefault      = Fmodule | Flevel | Fshortfile | FstdFlags
...
var Std = New(os.Stderr, "", 1, Fdefault)
...
func (l *Logger) formatHeader(buf *bytes.Buffer, t time.Time, file string, line int, lvl int, reqId string) {
	...
	if l.flag&(Fdate|Ftime|Fmicroseconds) != 0 {
		if l.flag&Fdate != 0 {
		...
		if l.flag&(Ftime|Fmicroseconds) != 0 {
	...
	if l.flag&(Fshortfile|Flongfile) != 0 {
		if l.flag&Fshortfile != 0 {
...
func (l *Logger) Output(reqId string, lvl int, calldepth int, s string) error {
	...
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.flag&(Fshortfile|Flongfile|Fmodule) != 0 {
